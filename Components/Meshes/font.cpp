...
// ========================================= DISCLAIMER ================================================================
// ============================= THIS IS GOT FROM STM32 font library ===================================================
// =====================================================================================================================
...
#define FONT_SUPPORTED_CHARACTERS 95

uint8_t asc2_1608[FONT_SUPPORTED_CHARACTERS][FONT_SIZE_16] = {
...
static inline uint32_t ushr(uint32_t value, uint8_t count) {
    return value >> count;
}

static inline uint32_t shl(uint32_t value, uint8_t count) {
    return value << count;
}

static inline uint8_t xbit_u32(uint32_t value, uint8_t index) {
    return ushr(value, index) & 1u;
}

void frame_draw_char(Frame frame, Color color, uint32_t x, uint32_t y, char ch) {
    uint8_t temp = 0;

    uint8_t num = (uint8_t) ch - ' ';

    for (uint32_t pos = 0; pos < FONT_SIZE_16; pos++) {
        temp = asc2_1608[num][pos];

        for (uint32_t bit = 0; bit < FONT_SIZE_16 / 2; bit++) {
            if (xbit_u32(temp, bit)) {
                frame[y + pos][x + bit] = color;
            }
        }
    }
}

void frame_draw_string(Frame frame, Color color, uint32_t x, uint32_t y, const char* str) {
    uint32_t length = static_cast<uint32_t>(std::strlen(str));
    for (uint32_t k = 0; k < length; k++) {
        uint32_t xc = x + FONT_SIZE_16 / 2 * k;
        frame_draw_char(frame, color, xc, y, str[k]);
    }
}
